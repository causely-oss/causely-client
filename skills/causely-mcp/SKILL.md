...
| "What's wrong with X?" (known degraded, need deep dive) | `triage(entity_name=)` | Structured narrative with portal links |
| "Full picture of X" (SLOs, deps, metrics) | `get_service_summary(service=)` | All-in-one: symptoms + RCs + SLOs + metrics + deps + events + logs |

**`triage` is for deep investigation, not simple health checks.** For "is X healthy?" use `get_service_summary` or `get_environment_health(services=["X"])` — they're faster and don't require pre-resolved entity IDs.
...
| "What's wrong with X?" / deep investigation | `triage(entity_name=)` | `get_logs` if description generic |
...
| "How's the team doing?" | `team_health(team=)` | `get_service_summary` per degraded service |
...
**Deep investigation (known degraded) → `triage` (1 call)**
```
triage(entity_name="<service>")
  → structured narrative with portal links
  → root cause, symptoms, impacted_services, impacted_customers, remediation
  → only use when you need the narrative format or have a root_cause_id
```
...
2. If degraded and need deep narrative: `triage(entity_name="<service>")`
...
2. No MCP tool calls needed unless the user also wants live triage data

### 🏢 Team standup
1. `team_health(team="<team>")` — returns degraded services first
2. For each degraded service: `get_service_summary(service=)` for full context
...
- **`get_service_summary` for health checks, `triage` for deep investigation.** Don't use triage for "is X healthy?" — it's slower and meant for degraded services.
...
- **`alert_state_filters` overrides `active_only`.** Use `["firing","resolved"]` to see both states regardless of `active_only` setting.
- **`name_lookup` for name resolution, `get_entities` for listing.** Resolve names with `name_lookup`; use `get_entities` to filter by type, cluster, or namespace.
